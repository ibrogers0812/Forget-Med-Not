...
def test_remove_pharmacy(client, mocker):
    """
    Test case to check if a user can successfully remove a pharmacy's information.
    """
    ...
    response = client.post('/remove_pharmacy', data=delete_data, follow_redirects=True)
    ...
    search_result = next((ph for ph in pharmacy_list if ph["name"] == "Test Pharmacy"), None)

    assert search_result is None, "Test Failed: Pharmacy information was not removed."
